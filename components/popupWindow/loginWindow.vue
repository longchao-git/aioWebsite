      ...
      <div>
        ...
        <div class='loginClass'>
          <div v-if='type===2&&orderAddrList' class='login_input'>
            ...
            <el-select v-model='addr_id' clearable filterable
          ...
          <div v-if='type===2' class='login_input'>
            <div>{{ $t(`depago`) }}</div>
            <el-select v-model='code' @change="bindCode" clearable filterable
                       :placeholder="$t('loginOrRegister.placeholder')[1]" style='flex: 1'>
          ...
          <div class='login_input'>
            ...
            <el-button type='primary' @click='handleChangeType(9)' v-if='!card_id'>{{ $t(`bank.name`) }}
            </el-button>
          ...
          <div v-if='type===2&&code==1&&orderInfo.hongbao_list' class='login_input'>
            <div>{{ $t(`红包`) }}</div>
            <el-select v-model='hongbao_id' filterable clearable
                       :placeholder="$t('loginOrRegister.placeholder')[1]" style='flex: 1'>
              <el-option v-for='(item, index) in orderInfo.hongbao_list' :key='index'
                         :disabled="item.is_canuse != 1" :value='item.hongbao_id'
                         :label="$t(`满`) +'€' +item.min_amount+$t('可减')+ '€'+item.amount"></el-option>
          ...
          <div v-if='type===2&&code==1&&orderInfo.coupon_list' class='login_input'>
            <div>{{ $t(`优惠券`) }}</div>
            <el-select v-model='coupon_id' filterable clearable
                       :placeholder="$t('loginOrRegister.placeholder')[1]" style='flex: 1'>
              <el-option v-for='(item, index) in orderInfo.coupon_list' :key='index'
                         :disabled="item.is_canuse != 1" :value='item.coupon_id'
                         :label="$t(`满`)  + `€${item.order_amount}`+$t('可减')+`€${item.coupon_amount}`"></el-option>
          ...
          <div v-if='type===2&&code==1&&orderInfo.cards&&orderInfo.cards.length>0' class='login_input'>
            ...
            <el-select v-model='peicard_id' filterable clearable :disabled="orderInfo.peicard_id!=0"
                       :placeholder="$t('loginOrRegister.placeholder')[1]" style='flex: 1'>
              <el-option v-for='(item, index) in orderInfo.cards' :key='index' :value='item.card_id'
                         :label="$t(`每单立减`) + `${item.reduce}€`+$t('配送费,价格')+`${item.amount}`"></el-option>
          ...
          <div v-if='type===2&&orderInfo.coupon_list&&coupon_id' class='line22 mt1'
            ...
            <template v-for="(item,index) in orderInfo.coupon_list">
              <div v-if="coupon_id==item.coupon_id" style="color: #ee8080">-€{{ item.coupon_amount }}
              </div>
            </template>
          </div>
          <div v-if='type===2&&peicard_id&&orderInfo.cards&&(orderInfo.peicard_id==0||!orderInfo.peicard_id) '
               class='line22 mt1' style='width: 100%;display: flex;justify-content: space-between;'>
            <div class='' style='width: 126px;text-align: right'>
              {{ $t(`购买配送会员卡`) }}
            </div>
          ...
          <div v-if='type===2&&orderInfo.support_first_share == 0 && orderInfo.first_order == 1'
               class='line22 mt1' style='width: 100%;display: flex;justify-content: space-between;'>
            <div class='' style='width: 126px;text-align: right'>
              {{ $t(`首单优惠`) }}
            </div>
            <el-switch v-model="isFirst" :active-value='1' :inactive-value='0' active-color="#ee8080"
          ...
          <div v-if='type===2' class='line22 mt1'
            ...
            <div v-if='orderInfo.youhui&&orderInfo.youhui.length>0'
